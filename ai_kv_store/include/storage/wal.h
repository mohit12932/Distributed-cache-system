// ai_kv_store/include/storage/wal.h
// ────────────────────────────────────────────────────────────────
...
enum class WALRecordType : uint8_t {
    kPut    = 0x01,
    kDelete = 0x02,
    kBatch  = 0x03,  // Multiple operations in one fsync
};
...
public:
    ...
    // Append a batch of records with a single fsync. Thread-safe.
    ...
    bool Sync() {
        ...
        file_.flush();
        // On production systems, call fdatasync() on the fd
        return file_.good();
    }
...
private:
    // Frame format: [CRC32:4][PayloadLength:4][Payload:N]
    ...
    static std::string Serialize(const WALRecord& rec) {
        // Format: [type:1][seq:8][key_len:4][key][val_len:4][value]
        std::string buf;
    ...
    static uint32_t ComputeCRC(const std::string& data) {
        // Simplified CRC — in production use absl::crc32c_t
        uint32_t crc = 0;
...
public:
    ...
    // Replay all valid records. Stops at first corruption.
    size_t Replay(RecordCallback cb) {
        ...
        while (file.good() && !file.eof()) {
            ...
            if (computed != stored_crc) break;  // Corruption detected
